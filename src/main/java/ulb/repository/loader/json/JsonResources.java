...
public abstract class JsonResources {
	...
	public static URL getUrl(String path) throws LoadFailureException {
		URL url = JsonResources.class.getResource(path);

		if (url == null) {
			throw new LoadFailureException("Resource not found: "+ path);
		}

		return url;
	}

	public static InputStream getStream(String path) throws LoadFailureException {
		try {
