...
public class BattleWindowGraphicsHelper {
	...
	public void linkUI(ProgressBar playerBugemonHPBar, ProgressBar opponentHPBar, Label playerBugemonHPNumber,
		...
		this.ui = new BattleUIComponents(playerBugemonHPBar, opponentHPBar, playerBugemonHPNumber, opponentHPNumber,
				playerBugemon, opponentBugemon, playerBugemonLabel, opponentBugemonLabel, playerLevelLabel,
	...
	public void showLogMessages(List<String> logs) {
		List<String> visibleLogs = logs == null ? List.of() :
		...
		String allMessages = visibleLogs.stream().map(message -> wrapText(message, 35)).collect(Collectors.joining("\n"
		));
	...
	public void displayMessagesSequentially(List<String> rawLogs, Integer hpAfterFirstActionSelf,
		...
		List<BattlePhaseStep> phaseSteps = buildPhaseSteps(phases, currentSnapshot, finalSnapshot,
				hpAfterFirstActionSelf, hpAfterFirstActionOpponent);
	 ...
	 */
	private List<BattlePhaseStep> buildPhaseSteps(List<List<String>> phases, BattleSnapshot currentSnapshot,
												  BattleSnapshot finalSnapshot, Integer hpAfterFirstActionSelf,
		...
		for (int i = 0; i < phaseCount; i++) {
			...
			if (phaseCount == 1) {
				...
				visual = buildFirstPhaseVisual(messages, currentSnapshot, finalSnapshot, hpAfterFirstActionSelf,
						hpAfterFirstActionOpponent);
			} else if (i == phaseCount - 1) {
				visual = new BattlePhaseVisual(finalSnapshot, null, null);
			} else {
				visual = buildIntermediateVisual(currentSnapshot, finalSnapshot, hpAfterFirstActionSelf,
						hpAfterFirstActionOpponent);
			}
	 ...
	 */
	private BattlePhaseVisual buildFirstPhaseVisual(List<String> messages, BattleSnapshot currentSnapshot,
													BattleSnapshot finalSnapshot, Integer hpAfterFirstActionSelf,
													Integer hpAfterFirstActionOpponent) {
		BattleSnapshot phaseSnapshot = isSwapPhase(messages) && finalSnapshot != null ? finalSnapshot :
				currentSnapshot;
	...
	private boolean isSwapMessage(String message) {
		return message != null && (message.startsWith("Tu as envoyé ") || message.startsWith("L'adversaire a envoyé ") || message.contains("a été envoyé "));
	}
	 ...
	 */
	private BattlePhaseVisual buildIntermediateVisual(BattleSnapshot currentSnapshot, BattleSnapshot finalSnapshot,
	...
	private boolean hasOpponentSwitched(BattleSnapshot currentSnapshot, BattleSnapshot finalSnapshot) {
		if (currentSnapshot == null || finalSnapshot == null || currentSnapshot.opponentBugemon() == null || finalSnapshot.opponentBugemon() == null) {
			return false;
		}
	 ...
	 */
	private void displayPhaseSequence(List<BattlePhaseStep> phaseSteps, int phaseIndex,
									  Supplier<BattleSnapshot> currentSnapshotSupplier,
									  Consumer<BattleSnapshot> battleRenderer, Runnable onComplete) {
		...
		displayPhase(currentStep.messages(), 0, currentStep.visual(), currentSnapshotSupplier, battleRenderer,
				() -> displayPhaseSequence(phaseSteps, phaseIndex + 1, currentSnapshotSupplier, battleRenderer,
						onComplete));
	}
	 ...
	 */
	private void displayPhase(List<String> messages, int index, BattlePhaseVisual visual,
							  Supplier<BattleSnapshot> currentSnapshotSupplier,
							  Consumer<BattleSnapshot> battleRenderer, Runnable onComplete) {
		...
		pause.setOnFinished(event -> displayPhase(messages, index + 1, visual, currentSnapshotSupplier, battleRenderer
				, onComplete));
		pause.play();
	}
	 ...
	 * @param battleRenderer callback that applies a snapshot to the UI
	 */
	private void applyPhaseVisual(BattlePhaseVisual visual, Supplier<BattleSnapshot> currentSnapshotSupplier,
	 ...
	 */
	private void playAttackAnimationIfNeeded(String message, BattlePhaseVisual visual,
											 Supplier<BattleSnapshot> currentSnapshotSupplier) {
		if (message == null || !message.contains(" a utilisé ")) {
			return;
		}

		BattleSnapshot displayedSnapshot = visual.snapshot() != null ? visual.snapshot() :
		...
		boolean playerMatches =
				displayedSnapshot.playerBugemon() != null && attackerName.equals(displayedSnapshot.playerBugemon().name());
		boolean opponentMatches =
				displayedSnapshot.opponentBugemon() != null && attackerName.equals(displayedSnapshot.opponentBugemon().name());
	...
	private void playAmbiguousAttackAnimation(BattlePhaseVisual visual, BattleSnapshot previousSnapshot) {
		if (firstAttackWasPlayer != null) {
			...
			playAttackAnimation(secondAttackerIsPlayer ? ui.playerBugemon() : ui.opponentBugemon(),
					secondAttackerIsPlayer);
			return;
		}
	 ...
	 * @return {@code true} if the player attacked first, {@code false} if the opponent attacked first, or {@code null
	 * } if unknown
	 */
	private Boolean inferAttackerFromHPChange(BattlePhaseVisual visual, BattleSnapshot previousSnapshot) {
		if (visual == null || previousSnapshot == null || visual.snapshot() == null || previousSnapshot.playerBugemon() == null || previousSnapshot.opponentBugemon() == null || visual.snapshot().playerBugemon() == null || visual.snapshot().opponentBugemon() == null) {
			return null;
		}
		...
		int displayedOpponentHP = visual.opponentHp() != null ? visual.opponentHp() :
				visual.snapshot().opponentBugemon().hp();
	...
	/**
	 * Associates a list of log messages with the visual state that should be active while those messages are
	 * displayed.
	 *
	...
	private record BattleUIComponents(ProgressBar playerBugemonHPBar, ProgressBar opponentHPBar,
	                                  Label playerBugemonHPNumber, Label opponentHPNumber, ImageView playerBugemon,
	                                  ImageView opponentBugemon, Label playerBugemonLabel, Label opponentBugemonLabel,
	                                  Label playerLevelLabel, Label opponentLevelLabel, VBox messageBox,
	                                  Label battleLog) {
	}
}
