...
public class AbilityTest {
	...
	public void correctDamageFormulaNoCrit() {
		Random random = RandomTestUtils.findRandom((rand) -> {
			return rand.nextDouble() > Ability.CRITICAL_HIT_CHANCE;
		});
		...
		BugemonSpecies testSpecies = new BugemonSpecies("id", "name", Type.FLORA, new Stats(100, 100, 100, 100),
				new AbilitySet(abilityA, abilityB, abilityC), "sprite", true);
	...
	public void correctDamageFormulaWithCrit() {
		...
		float expectedDefenseFactor = 100.0f / (100.0f + oppositeBugemon.getDefense()); // = 0.5
		float expectedRawDamage = abilityA.getPower() * expectedAttackFactor * expectedDefenseFactor; // = 10
		int expectedDamage = Math.round(expectedRawDamage * Ability.CRITICAL_HIT_FACTOR); // with crit, no
		// effectiveness
	...
	public void effectivenessMessageIsCorrectWhenSuperEffective() {
		...
		BugemonSpecies testSpecies = new BugemonSpecies("id", "name", Type.AQUA, new Stats(100, 100, 100, 100),
				new AbilitySet(abilityA, abilityB, abilityC), "sprite", true);
	...
	public void effectivenessMessageIsCorrectWhenNotEffective() {
		...
		BugemonSpecies testSpecies = new BugemonSpecies("id", "name", Type.LITHO, new Stats(100, 100, 100, 100),
				new AbilitySet(abilityA, abilityB, abilityC), "sprite", true);
